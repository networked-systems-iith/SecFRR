...
class FlowSelector:
    ...
    timeout=2, seed=1, window_size=10, bin_time=0.08):
        ...
        logger.setup_logger('flowselector', log_dir+'/flowselector-logs/'+pcap+'.log', level=log_level)
        self.log = logging.getLogger('flowselector')

        # Logger for the sliding window
        logger.setup_logger('sliding_window', log_dir+'/slidingwindow-logs/'+pcap+'.log', level=log_level)
        ...
        self.outfile_dump_prefix = pcap+'flowselector_dump'
        self.dump_last_ts = 0

        # Some variables used for monitoring
        self.ssd_dic = {}
        self.bogus_trace = {}
        self.retr = 0 # used to maintain the number of retransmissions per window
    ...
    def update_window(self, packet):
        ...
        # If we must move to the next bin
            ...
            #print 'lol_uw0',packet.ts,self.registers['sw_time'][prefix_id]
    ...
    def process_packet(self, packet):
        ...
        # If the flow is not monitored
            ...
            if (curflow_key == 0 or packet.ts - curflow_ts > self.timeout) and not packet.fin_flag:
                ...
                # Initialize fs and fd feature since this is the start of the new flow
